...
// SECURITY: Escape HTML entities to prevent XSS in customer emails
...
export async function sendOrderConfirmationEmail(
  ...
  const itemsList = (order?.items || [])
    .map(
      (item) =>
        `<tr>
          <td style="padding:8px 12px;border-bottom:1px solid #ffe5ec;font-size:0.88rem;color:#1a1a2e">${escapeHtml(item.name) || 'Item'}</td>
