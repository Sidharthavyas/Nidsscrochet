 ...
 *
 * ⚠️  M-3 WARNING: This Map lives in the Node.js process memory.
 * On Vercel (serverless), each function instance has its own Map, and it
 * resets on every cold start. This means the counter is NOT shared across
 * concurrent instances — real protection requires a Redis-backed store
 * (e.g. @upstash/ratelimit with Vercel KV / Upstash Redis).
 */
...
/**
 * File upload validation — MIME type check only (content-agnostic).
 * ⚠️  MIME can be spoofed by the client. Always pair with validateMagicBytes().
...
/**
 * L-2: Magic bytes validation — reads the first bytes of a file buffer and
 * verifies they match a known image format signature.
 * This prevents attackers from renaming arbitrary files (PHP, HTML, scripts)
 * to `.jpg` and bypassing the MIME-type check.
 *
 * @param {Buffer} buffer - First N bytes of the uploaded file (minimum 12 bytes)
 * @returns {object} - { valid: boolean, error?: string, mimeType?: string }
 */
export const validateMagicBytes = (buffer) => {
    ...
    return {
        valid: false,
        error: 'File content does not match a supported image format (JPEG, PNG, GIF, WebP)',
    };
};
